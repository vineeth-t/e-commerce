export function FilterReducer(state, action) {
  switch (action.type) {
    case "FAST_DELIVERY":
      return {
        ...state,
        showJCAssured: !state.showJCAssured,
      };
...
export function getFilterData(productList, { toggleInventory, showJCAssured }) {
  if (showJCAssured === true && toggleInventory === true) {
    ...
    return productList.filter(
      (product) => product.fastDelivery === true && product.inStock === true
    );
