...
export function Cart(){
    let totalAmount=0.0;
    const {setWishlistItems,state,dispatch}=useStateContext();
    return (
        <div className='cart-hero'>
            <div className='cart-holder'>
                    ...
                    <div>
                        <div>
                             {state.cartItems.map(({id,name,image,price})=>{
                                        totalAmount=totalAmount + ( (state.quantity)*parseInt( price, 10 ))
                                    return (
                                    <div className='cart-container'>
                                                <div className='cart-item-container' >   
                                                        ...
                                                        <div className='cart-item-details'>
                                                            <div>
                                                            <h5>{name}</h5>
                                                            <h6>Rs/-{(state.quantity)*price}</h6>
                                                            
                                                            </div>
                                                            <div className='button-qty'>
                                                                <button onClick={()=>dispatch({type:"Decrement"})} className='btn-primary'>-</button>
                                                               <span className='quantity'>{state.quantity}</span> 
                                                                <button onClick={()=>dispatch({type:"Increment"})} className='btn-primary'>+</button>
                                                            </div>
                                                            <div>
                                                                 <button className='btn-wishlist' onClick={()=>deleteItem({id,name,image,price},setWishlistItems,"Cart")}>Move to Wishlist</button>
                                                            </div>
                                                        </div> 
                                                    </div> 
                                    </div>
